...
def welcome():
    ...
    print(Fore.MAGENTA + Style.BRIGHT + "Trivago - The Best Rooms For the Best Prices!")
...
def hotels_file_creation():
    ...
    hotel = ['isrotel', 'fattal']
    temp_directory = tempfile.gettempdir()
    hotels = open(temp_directory + "/hotels.txt", "w")

    # hotels = {"1": {"name": ["isrotel", "fattal"], "room": 4, "guests": 3,"cost": random.randrange(250, 450), "day": days},{"1": {"name": ["isrotel", "fattal"], "room": 4, "guests": 3,"cost": random.randrange(250, 450), "day": days}}
    hotels = {1: {'name': hotel, 'room': 1, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              2: {'name': hotel, 'room': 2, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              3: {'name': hotel, 'room': 3, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              4: {'name': hotel, 'room': 4, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              5: {'name': hotel, 'room': 1, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              6: {'name': hotel, 'room': 2, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              7: {'name': hotel, 'room': 3, 'guests': 3,'cost': random.randrange(250, 450), 'day': days},
              8: {'name': hotel, 'room': 4, 'guests': 3,'cost': random.randrange(250, 450), 'day': days}}
    print(hotels[8])

    # hotels = open(temp_directory + "/hotels.txt", "a")
    # hotels.write("{}".format(json.dumps({"hotel": ["isrotel", "fattal"], "room": 4, "guests": 3,"cost": random.randrange(250, 450), "day": days})))
    # days_file = open(temp_directory + "/hotels.txt", 'r')
    # hotels.write(json.dumps({'hotel': 'Isrotel', 'room': 1, 'guests': 2, 'cost': random.randrange(250, 450), 'day': days}))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Isrotel', 'room': 2, 'guests': 2, 'cost': random.randrange(250, 450), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Isrotel', 'room': 3, 'guests': 3, 'cost': random.randrange(300, 500), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Isrotel', 'room': 4, 'guests': 3, 'cost': random.randrange(300, 500), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Fattal', 'room': 1, 'guests': 2, 'cost': random.randrange(350, 550), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Fattal', 'room': 2, 'guests': 2, 'cost': random.randrange(350, 550), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Fattal', 'room': 3, 'guests': 3, 'cost': random.randrange(400, 600), 'day': days})))
    # hotels.write("\n{}".format(json.dumps({'hotel': 'Fattal', 'room': 4, 'guests': 3, 'cost': random.randrange(400, 600), 'day': days})))
    # hotels.close()

def reservation_file_creation():
    temp_directory = tempfile.gettempdir()
    reservation = open(temp_directory + "/reservation.txt", "w")
    # reservation.close()
...
def check_available_rooms():
    print(Style.BRIGHT + "\nAvailable Rooms:")
    temp_directory = tempfile.gettempdir()
    for line in open(temp_directory + "/hotels.txt", "r").readlines():
        hotels = line.replace("'", "\"")
...
def select_departure_day(arrival_day, nights):
    ...
    temp_directory = tempfile.gettempdir()
    with open(temp_directory + "/hotels.txt") as f:
        line = f.readline()
        line = line.replace("'", "\"")
...
def available_rooms_empty_file_creation():
    temp_directory = tempfile.gettempdir()
    available_rooms = open(temp_directory + "/available_rooms.txt", "w+")
    available_rooms.write("")
    available_rooms.close()
    available_rooms = open(temp_directory + "/available_rooms_isrotel.txt", "w+")
    available_rooms.write("")
    available_rooms.close()
    available_rooms = open(temp_directory + "/available_rooms_fattal.txt", "w+")
    available_rooms.write("")
    available_rooms.close()
...
def reservation():
    ...
    for line in open(temp_directory + "/hotels.txt", "r").readlines():
        hotels = line.replace("'", "\"")
        hotels = json.loads(hotels)
        available_rooms = open(temp_directory + "/available_rooms.txt", "a+")
    ...
    for line in open(temp_directory + "/available_rooms.txt", "r").readlines():
        hotels = line.replace("'", "\"")
        hotels = json.loads(hotels)
        available_rooms_fattal = open(temp_directory + "/available_rooms_fattal.txt", "a+")
        hotels['cost'] = hotels['cost']
    ...
    for line in open(temp_directory + "/available_rooms.txt", "r").readlines():
        hotels = line.replace("'", "\"")
        hotels = json.loads(hotels)
        available_rooms_isrotel = open(temp_directory + "/available_rooms_isrotel.txt", "a+")
        if breakfast == "yes":
    ...
    for i in range(4):
        if hotels_preferation == 1:
            for line in open(temp_directory + "/available_rooms_fattal.txt", "r").readlines():
                ...
                if re.search("Fattal", str(available_rooms_fattal)):
                    if re.search(str(costs_fattal[i]), str(available_rooms_fattal)):
                        if re.search(str(days_list), str(available_rooms_fattal)):
                            reservation_file = open(temp_directory + "/reservation.txt", "a+")
                            if i < rooms:
                                ...
                                print(Fore.LIGHTBLUE_EX + "Room: " + str(available_rooms_fattal['room']) + ", Hotel: " +
                                    ...
                                    available_rooms_fattal['cost'] * nights + breakfast_price * nights) + "$")
        if hotels_preferation == 2:
            for line in open(temp_directory + "/available_rooms_isrotel.txt", "r").readlines():
...
def delete_reservation_from_main_hotels_file(arrival_day, departure_day ,hotel_name, costs, rooms):
    temp_directory = tempfile.gettempdir()
    i = 0
    for line in open(temp_directory + "/hotels.txt", "r").readlines():
        # hotels = line.replace("'", "\"")
        hotels = json.loads(line)
        index_arrival_day = hotels['day'].index(str(arrival_day).title())
        index_departure_day = hotels['day'].index(str(departure_day).title())
        for cost in costs:
            if hotel_name == "Fattal":
                if re.search(str(hotel_name), str(hotels)):
                    if re.search(str(cost), str(hotels)):
                        if i < rooms:
                            ...
                            del hotels['day'][0:-1]
                            hotels.write(hotels)
            if hotel_name == "Isrotel":
                if re.search(str(hotel_name), str(hotels)):
                    if re.search(str(cost), str(hotels)):
                        if i < rooms:
                            i = i + 1
            if hotel_name != "Fattal" and hotel_name != "Isrotel":
                        if re.search(str(cost), str(hotels)):
                            if i < rooms:
                                i = i + 1
    # hotels.close()
    ...
    # finish = decision()
    # if finish == 1:
    #     reservation = open(temp_directory + "/reservation.txt", "r+")
...
###### main
hotels_file_creation()
# reservation_file_creation()
# welcome()
# menu()
